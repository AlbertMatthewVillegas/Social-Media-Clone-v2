  ...
  return (
    <div className="flex flex-row gap-4 p-4">
      {posts.map((post: PostEntity) => (
        <div
          className="relative border rounded-xl w-100 h-160 overflow-hidden group"
          key={post.postId}
        >
          <PopUpProvider>
            <PostContainer post={post} />
          </PopUpProvider>
          {/* TODO: create a function component that pops up and redirects to /p/postId later */}
        </div>
      ))}
    </div>
  );
...
function PostContainer({ post }: { post: PostEntity }) {
...
function CommentsDisplay({ post }: { post: PostEntity }) {
  return (
...
function LikeDisplay({ post }: { post: PostEntity }) {
  return (
    <div className="flex items-center gap-1">
